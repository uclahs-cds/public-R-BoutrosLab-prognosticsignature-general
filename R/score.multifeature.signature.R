...
score.multifeature.signature <- function(expression.data, survival.data, feature.names, feature.weights, feature.thresholds = NULL, score.cutoff = NULL) {
	...
	if (! is.null(feature.thresholds)) { if (length(feature.weights) != length(feature.names)) { warning('If feature weights are specified, there must be an equal number of weights and features'); return(NA); } }
